...
export function searchBooksAction(searchItem) {
  return async dispatch => {
    const response = await axios.get('/books/search/' + searchItem.itemVal);
    console.log('------------------>',response)
    if (response.status === 200) {
      dispatch({ type: 'Search_BOOK_LIST', payload: response.data });
    } else {
      dispatch({ type: 'ERROR_Search_BOOK_LIST', payload: '' });
    }
  };
}
